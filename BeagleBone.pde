...
ArrayList<BeagleBone> cortexList = new ArrayList<BeagleBone>();

public void addBeagleBones(LX lx) {
    lx.addOutput(new BeagleBone(lx, "192.168.1.80", 1, concatenateChannels(1)));
    lx.addOutput(new BeagleBone(lx ,"192.168.1.81", 2, concatenateChannels(2)));
}
...
public class BeagleBone extends LXOutput {
  ...
  BeagleBone(LX lx, String _host, int _boardNum, int[] _pointIndices) {
    super(lx);
    this.host = _host;
